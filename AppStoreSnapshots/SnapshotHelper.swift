...
open class Snapshot: NSObject {
    ...
    class func setLocale(_ app: XCUIApplication) {
        ...
        do {
            let trimCharacterSet = CharacterSet.whitespacesAndNewlines
            locale = try String(contentsOf: path, encoding: .utf8).trimmingCharacters(
                in: trimCharacterSet)
        } catch {
    ...
    open class func snapshot(_ name: String, timeWaitingForIdle timeout: TimeInterval = 20) {
        ...
        if Snapshot.waitForAnimations {
            ...
            guard let app = self.app else {
                NSLog(
                    "XCUIApplication is not set. Please call setupSnapshot(app) before snapshot().")
                return
            }
            ...
            do {
                ...
                NSLog(
                    "Problem writing screenshot: \(name) to \(screenshotsDir)/\(simulator)-\(name).png"
                )
                NSLog(error.localizedDescription)
            }
        #endif
