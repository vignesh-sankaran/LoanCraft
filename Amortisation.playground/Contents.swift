...
func calculateAmortizationSchedule(loanAmount: Double, annualInterestRate: Double, loanTermYears: Int) -> [AmortizationSchedule] {
    // Convert inputs
    ...
    let monthlyPayment = loanAmount * (monthlyInterestRate * pow(1 + monthlyInterestRate, Double(totalPayments))) / (pow(1 + monthlyInterestRate, Double(totalPayments)) - 1)
...
let schedule = calculateAmortizationSchedule(loanAmount: loanAmount, annualInterestRate: annualInterestRate, loanTermYears: loanTermYears)

// Print the amortization schedule
for entry in schedule {
    print("Month \(entry.month): Total Payment: \(String(format: "%.2f", entry.totalPayment)), Principal: \(String(format: "%.2f", entry.principal)), Interest: \(String(format: "%.2f", entry.interest)), Remaining Balance: \(String(format: "%.2f", entry.remainingBalance))")
}
